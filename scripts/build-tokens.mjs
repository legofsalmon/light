...
export function generate(doc = JSON.parse(readFileSync(TOKENS_JSON, 'utf8'))) {
  ...
  const lines = [];
  const touch = [];
  // Color collection, Light mode: a colour token may carry a second value for
  // a screen in daylight (Display → colours). Semantic tokens carry it, so
  // theme.css reads one set of names in both modes; a primitive carries one
  // only where a component reads the primitive itself.
  const light = [];
  const ts = { color: {}, colorLight: {}, space: {}, radius: {}, size: {}, sizeTouch: {}, motion: {}, opacity: {}, ratio: {}, type: {} };
  ...
  // a primitive may alias another (alpha/accent-8 is alpha/cyan-8 under the
  // name the design uses), so it resolves like a semantic token does
  const lightMode = (path, t, name) => {
    const v = t.$extensions?.['com.light.figma']?.modes?.Light;
    if (v === undefined) return;
    ts.colorLight[path.join('/')] = resolve(v);
    light.push(`  ${name}: ${asCss(v)};`);
  };
  emit('primitive', 'primitives — the raw values; components use the semantic names below', (path, t, name) => {
    ts.color[path.join('/')] = resolve(t.$value);
    lightMode(path, t, name);
  ...
  // two number collections: a pad face's opacities, and the fractions a
  // window is split by — bare numbers in both files
  emit('opacity', 'opacity (Figma: Opacity) — the only opacities a pad face may carry', (path, t) => {
    ts.opacity[path.join('/')] = Number(t.$value);
    return String(t.$value);
  });
  emit('ratio', 'ratio (Figma: Ratio) — fractions of a window or an area', (path, t) => {
    ts.ratio[path.join('/')] = Number(t.$value);
    return String(t.$value);
  });
  ...
  const css =
    ...
    `/* Color collection, Light mode — a screen in daylight (Display → colours).\n   Only the semantic names change; theme.css reads the same names and the rest\n   follows. The stage and the meters stay dark: a stage is dark. */\n:root[data-theme="light"] {\n  color-scheme: light;\n${light.join('\n')}\n}\n`;
...
// resolved hex; opacities and ratios are fractions.\n`;
  ...
  const tsOut =
    ...
    `/** the Light-mode values of the colours that have one, resolved */\nexport const colorLight = ${lit(ts.colorLight)} as const;\n\n` +
    ...
    `/** a pad face's opacities: rest, hover, playing */\nexport const opacity = ${lit(ts.opacity)} as const;\n\n` +
    `/** fractions of a window or an area */\nexport const ratio = ${lit(ts.ratio)} as const;\n`;
